...
export default function AdminPlanes() {
  ...
  return (
    <div style={est.contenedor}>
                ...
                <tr key={plan.id} style={{ opacity: plan.activo ? 1 : 0.5 }}>
                  ...
                  <td style={est.td}>
                    <span style={{
                      ...
                      background: plan.activo ? '#ecfdf5' : '#fef2f2', color: plan.activo ? C.exito : C.peligro,
                    }}>
                  ...
                  <td style={est.td}>
                    <div style={{ display: 'flex', gap: 8 }}>
                      ...
                      <button style={{ ...est.btnAccion, color: plan.activo ? C.peligro : C.exito }} onClick={() => toggleActivo(plan)}>
      ...
      {modalAbierto && (
        <CodeplexModal open={modalAbierto} onClose={() => setModalAbierto(false)} title={planEditando.id ? 'Editar Plan' : 'Nuevo Plan'} maxWidth="md">
...
const est = {
  ...
  badgeDest: { background: '#fef3c7', color: '#92400e', padding: '2px 6px', borderRadius: 4, fontSize: '0.7rem', marginLeft: 6 },
  btnPrimario: { padding: '10px 24px', border: 'none', borderRadius: 8, cursor: 'pointer', fontWeight: 700, fontSize: '0.85rem', background: C.primario, color: '#fff' } as React.CSSProperties,
  btnSecundario: { padding: '10px 24px', border: `1px solid ${C.borde}`, borderRadius: 8, cursor: 'pointer', fontWeight: 600, fontSize: '0.85rem', background: '#fff', color: C.texto } as React.CSSProperties,
  ...
  input: { width: '100%', padding: '8px 12px', border: `1px solid ${C.borde}`, borderRadius: 8, fontSize: '0.9rem', outline: 'none', boxSizing: 'border-box' as const } as React.CSSProperties,
